				...
				for (int16_t input_y = -128; input_y < 128; input_y++) {
					...
					*marioZ(game) = float(*marioZ(game) + *marioZSlidSpd(game) * *marioFloorNormalY / 4.0);
					...
					if (check_if_inbounds(*marioX(game), *marioZ(game)) == true) {
						...
						*marioX(game) = float(*marioX(game) + *marioHSpd(game) * float(gSineTable[(uint16_t)(*marioFYaw(game)) >> 4] / 4.0));
						*marioZ(game) = float(*marioZ(game) + *marioHSpd(game) * float(gCosineTable[(uint16_t)(*marioFYaw(game)) >> 4] / 4.0));
